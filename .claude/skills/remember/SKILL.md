---
name: remember
description: Capture durable project decisions, user corrections, preferences, and verified lessons from the current conversation into this repository's project memory. Use for remember requests or newly established reusable knowledge about this project.
---
# Remember

Save a short evidence-bearing episode under `.claude/memory/episodes/`. These episodes
bridge sessions; they are not complete transcripts. Read
`.claude/skills/dream/references/memory-format.md` for the episode format.

This writes **project** memory for this repository only, never the user-level memory
directory under `~/.claude/projects/` and never the `.kiro/` kit's store.
If a fact belongs to the user across all projects rather than to this repository, say so
and do not file it here.
...
3. Write a unique UTF-8 Markdown file under `.claude/memory/episodes/`, named
   `<UTC-timestamp>-<topic-slug>.md`: the current time from the environment as
   `YYYYMMDDTHHMMSSZ`, then a short lowercase kebab-case slug naming the subject and
   matching the episode's title. Use the environment's time, not a date copied from
   these instructions. If that filename exists, make the slug more specific rather than
   adding a random suffix. Never overwrite an existing episode, and never rename one a
   dream has already processed.
...
5. Keep an episode around 100-300 words unless the evidence needs more. Store only
   ...
   Write one episode per distinct durable claim, usually one for the turn. Do not
   bundle unrelated claims into a single file, and do not split one claim across
   several. Do not modify active stores or mark it processed.
